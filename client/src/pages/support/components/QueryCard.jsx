...
import { ChevronDown, ChevronUp, User, ShieldCheck, Clock } from 'lucide-react'
import { Card, CardContent } from '@/components/ui/card'
import { Badge } from '@/components/ui/badge'

const statusConfig = {
  open: { color: 'bg-[var(--accent)] text-[var(--accent-foreground)]', label: 'Open' },
...
const QueryCard = ({ query }) => {
  ...
  const dateStr = new Date(query.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })
      ...
      >
        <div className="flex flex-col gap-1">
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <Badge variant="outline" className="border-border bg-background capitalize">
              {query.inquiry}
            </Badge>
            <span>• {dateStr}</span>
          </div>
              ...
              <div className="flex flex-row-reverse gap-3">
                ...
                <div className="space-y-2 text-right">
                    <div className="inline-block rounded-lg rounded-tr-none bg-primary/5 border border-primary/10 p-3 text-left text-sm text-foreground">
                        <p className="font-medium text-primary text-xs mb-1">Admin Response</p>
               ...
               <div className="ml-11 flex items-center gap-2 rounded-md border border-dashed border-accent bg-(--accent)/5 p-3 text-xs text-accent">
                  ...
                  <span>Our experts are reviewing your query. Expect a reply within 24 hours.</span>
               </div>
            )}
          </CardContent>
        </div>
