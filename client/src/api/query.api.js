...
export const updateQuery = async ({ payload, queryId }) => {
  const { data } = await api.patch(`query/${queryId}`, payload)
  return data
}

export const closeQuery = async ({queryId}) => {
  const { data } = await api.patch(`/query/${queryId}/close`)
  return data
}